...
---- Káº¿t KhÃºc Ui
function NotificationConfig(Text, Status, Duration)
    --[[
Success
Warning
Error
Info
    ...
    NotificationLibrary:SendNotification(Status, Text, Duration)
end
...
getgenv().HyperCahayas = function(p)
    ...
    local Library = Fluent:CreateWindow({
        Title = "LDQ HUB",
...
local Tabs = {
 Main = Library:AddTab({ Title = "Main", Icon = "home" }),
 M = Library:AddTab({ Title = "Item", Icon = "" }),
 Ss = Library:AddTab({ Title = "Stats", Icon = "plus-circle" }),
 RaceV4 = Library:AddTab({ Title = "Race V4", Icon = "chevrons-right" }),
 P = Library:AddTab({ Title = "PVP", Icon = "" }),
 R = Library:AddTab({ Title = "Dungeon", Icon = "swords" }),
 T = Library:AddTab({ Title = "Teleport", Icon = "palmtree" }),
 S = Library:AddTab({ Title = "Shop", Icon = "shopping-cart" }),
 D = Library:AddTab({ Title = "Devil Frut", Icon = "cherry" }),
 Misc = Library:AddTab({ Title = "Misc", Icon = "list-plus" }),
}
...
function AntiBan()
 ...
 local FarmSeting = Tabs.Main:AddSection("Farm Settings")
...
local InputFastAttack = Tabs.Main:AddInput("Input", {
    Title = "Nhập Tốc Độc đánh của bạn (mặc định là 0.15)",
    Default = "0.15",
    Placeholder = "0", "0.1", "0.15", "0.155", "0.16", "0.165", "0.17", "0.175", "0.18", "0.185",
...
InputFastAttack:OnChanged(function()
    _G.FastAttackDelay = InputFastAttack.Value
end)
spawn(function()
    while wait(.1) do
...
local ToggleBypassTP = Tabs.Main:AddToggle("ToggleBypassTP", {
    Title = "Bypass TP",
    Description = "Sẽ Reset Character và Dịch chuyển đến Set Spawn Point tùy hoàn đảo level của bạn",
    Default = false 
})
    ...
    local ToggleAwakeningRace = Tabs.Main:AddToggle("ToggleAwakeningRace", {
        Title = "Turn On V4 Race",
        Description = "Tự động bật RaceV4 (Chỉ dành ai nâng tộc để PVP)",
        Default = false 
    })
        ToggleAwakeningRace:OnChanged(function(Value)
       ...
       local ToggleSetSpawn = Tabs.Main:AddToggle("ToggleSetSpawn", {
        Title = "Set Spawn Point",
        Description = "Tự động Set Spawn khi bạn đang farm ở đảo nào đó",
        Default = false
     })
        SetSpawn:OnChanged(function(Value)
            _G.Set = Value
        end)
spawn(function()
   while wait() do
...
local ToggleFastAttack = Tabs.Main:AddToggle("ToggleFastAttack", {
    ...
    local Farm = Tabs.Main:AddSection("Farm Level,...")
   
    local ToggleFarmLevel = Tabs.Main:AddToggle("ToggleFarmLevel", {
        Title = "Farm Level",
        Description = "Farm đến Level Max thì thôi :) ",
        Default = false 
    })
    ...
    local ToggleFarmNearest = Tabs.Main:AddToggle("ToggleFarmNearest", {
        Title = "Farm Nearest",
        Description = "tự động farm chỗ khác mà không farm theo level",
        Default = false
     })
...
spawn(function()
 ...
 local ChestFarm = Tabs.Main:AddSection("Chest Farm")
    
    local ToggleFarmChest = Tabs.Main:AddToggle("ToggleFarmChest", {
        Title = "Farm Chest | Safe",
        Description = "Xài tween để lấy rương",
        Default = false 
    })
...
local ToggleChestBypass = Tabs.Main:AddToggle("ToggleChestBypass", {
    Title = "Farm Chest Bypass | Risk Kick",
    Description = "Khả năng bạn sẽ bị kick hoặc ăn ban",
    Default = false 
})
    ToggleChestBypass:OnChanged(function(Value)
...
local DoughBoss = Tabs.Main:AddSection("Dough Boss")
    
        local MobKilled = Main:AddSection("Killed")
    ...
    local ToggleBypassCakeIslandTeleport = Tabs.Main:AddToggle("ToggleBypassCakeIslandTeleport", {
        Title = "Bypass Cake Island Teleport",
        Description = "",
        Default = false
     })
        ToggleBypassCakeIslandTeleport:OnChanged(function()
            if BypassTP then
    ...
    local ToggleDoughtBoss = Tabs.Main:AddToggle("ToggleDoughtBoss", {
        Title = "Farm Cake Prince",
        Description = "Farm cái gì đó cho nhiệm vụ đánh DoughBoss",
        Default = false 
    })
        ToggleDoughtBoss:OnChanged(function(Value)
    ...
    local ToggleSpawnDoughtBoss = Tabs.Main:AddToggle("ToggleSpawnDoughtBoss", {
        Title = "Spawn Cake Prince",
        Description = "không biết tính năng này là gì nhưng sẽ tạo bánh Prince :O",
        Default = false
     })
        ToggleSpawnDoughtBoss:OnChanged(function(Value)
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner",Value)
    end)
    
    local ToggleFarmDoughKing = Tabs.Main:AddToggle("ToggleFarmDoughKing", {
        Title = "Farm Dough King",
        Description = "Đáng Boss Dough King phải không nhỉ ?",
        Default = false })
        ToggleFarmDoughKing:OnChanged(function(Value)
            _G.Autodoughking = value
            StopTween( _G.Autodoughking)
    end)

    local ToggleFarmDoughKingHop = Tabs.Main:AddToggle("ToggleFarmDoughKingHop", {
        Title = "Farm Dough King Hop",
        Description = "Đáng Dough King và chuyển server khác để đáng chắc kiếm Spidy quá",
        Default = false })
        ToggleFarmDoughKingHop:OnChanged(function(Value)
...
local BossSec = Tabs.Main:AddSection("Boss")
...
local BossName = Tabs.Main:AddDropdown("DropdownBoss", {
    Title = "Select Boss",
    Description = "Chọn boss tùy Sea bạn ở ví dụ 1,2 và 3",
    Values = bossCheck,
    Multi = false,
    Default = 1,
})
...
local ButtonRefeshBoss = Tabs.Main:AddButton("ButtonRefeshBoss", {
        Title = "Refesh Boss",
        Description = "",
        Callback = function()
        BossName:Clear()
...
local ToggleFarmBoss = Tabs.Main:AddToggle("ToggleFarmBoss", {
    Title = "Farm Boss",
    Description = "Farm bot mà bạn đã chọn á",
    Default = false 
})
    ToggleFarmBoss:OnChanged(function(Value)
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
    ...
    local ToggleFarmAllBoss = Tabs.Main:AddToggle("ToggleFarmAllBoss", {
        Title = "Farm All Boss",
        Description = "Tự động farm tất cả boss mà không cần lựa chọn",
        Default = false 
    })
        ToggleFarmAllBoss:OnChanged(function(Value)
            _G.AutoAllBoss = Value
        StopTween(_G.AutoAllBoss)
    end)
    
    local ToggleQuestBoss = Tabs.Main:AddToggle("ToggleQuestBoss", {
        Title = "Quest Boss (Beta)",
        Description = "Khi farm level thì bạn có lần farm boss khi boss xuất hiện tùy hoàn đảo level của bạn",
        Default = false
     })
        ToggleQuestBoss:OnChanged(function(Value)
            _G.Auto_Quest_Boss = Value
    end)

    local ToggleFarmAllBossHop = Tabs.Main:AddToggle("ToggleFarmAllBossHop", {
        Title = "Farm All Boss Hop",
        Description = "khi không còn boss nào farm thì bạn sẽ sang máy chủ khác để farm tiếp",
        Default = false 
    })
        ToggleFarmAllBossHop:OnChanged(function(Value)
...
spawn(function()
 ...
 local SettingFarmTwo  =  Main:AddSection("Setting Farm 2")
    
 local ToggleBringMob = Tabs.Main:AddToggle("ToggleBringMob", {
    Title = "Bring Mob",
    Description = "Gổm con Dummy 1 chỗ và khóa tại chỗ không cho đi đâu cả",
    Default = true 
})
    ToggleBringMob:OnChanged(function(Mag)
        _G.BringMonster = Mag
end)
...
_G.BringMode = "Normal"
local DropdownBring = Tabs.Main:AddDropdown("DropdownBring", {
    Title = "Bring Mob Mode",
    Description = "Tùy chế độ gôm Dummy",
    Values = Bring,
    Multi = false,
    Default = 1,
})
DropdownBoss:SetValue("Normal")
DropdownBoss:OnChanged(function(Value)
    _G.BringMode = Value
end)
spawn(function()
...
local ToggleBusoHaki = Tabs.Main:AddToggle("ToggleBusoHaki", {
    Title = "Buso Haki",
    Description = "Bypass haki để đáng boss có trái nào đó cần haki để đánh được",
    Default = true
 })
    ToggleBusoHaki:OnChanged(function(value)
        _G.AUTOHAKI = value
end)
...
local ToggleAutoClick = Tabs.Main:AddToggle("ToggleAutoClick", {
    Title = "Auto Click",
    Description = "tự động bấm màn hình cho combat khi không xài Fast Attack",
    Default = false 
})
...
Main:AddToggle("Remove Notifications Text","xóa text về ví dụ chém người,farm level,fullmoon,vân vân",false,function(NamfonGay)
...
Main:AddToggle("Remove Hit Sound And Level Up","xóa tiếng level up khi làm xong nhiệm vụ và tiếng Combat khi đáng",false,function(Remo)
_G.RemoveHit = Remo
end)
...
spawn(function()
    ...
    spawn(function()
      ...
      if _G.WhiteScreen then
        ...
        Main:AddToggle("White Screen","Giảm lag phần nhỏ",_G.WhiteScreen,function(value)
...
elseif _G.WhiteScreen == false then
    ...
    Main:AddToggle("Invisible Monsters","Bạn sẽ không thấy Dummy nhưng người khác thì có",_G.inv,function(value)
    _G.inv = value
...
spawn(function()
    ...
    Misc:AddButton("Hop To Lower Player",function()
        ...
        if not game:IsLoaded() then
            NotificationConfig("Game is loading","Warning",10)
        end
...
Misc:AddButton("Kaitun Cap", function(value)
    ...
    local FpsValue = Tabs.Misc:AddInput("Fps Input", {
        Title = "Fps Input",
        Default = "90",
        Placeholder = "Nothing",
        Numeric = true, -- Only allows numbers
...
spawn(function()
    ...
    InterfaceManager:SetFolder("WinningSmile")
    SaveManager:SetFolder("WinningSmile/blox-fruits")
